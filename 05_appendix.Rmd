...
```{r appendixG, echo=FALSE}
   ...
   #prep images
     trap.set.yr1         <-  srvy.set.details[ ,c(-1,-4:-18), ]
     addition         <-  c(8,"StRS")
     addition2        <-  c(11,"StRS")
     trap.set.yr1         <-  rbind(trap.set.yr1,addition)
     trap.set.yr1         <-  rbind(trap.set.yr1,addition2)
     names(trap.set.yr1)  <-  c("surveyset","Type")
     
     trap.set.yr1$count   <- 100
     trap.set.yr1         <- trap.set.yr1%>%  # set background color
                         mutate(get.color = ifelse(Type %in% c("OffStd") , "gray",  "white"))
     trap.set.yr1$get.color[trap.set.yr1$Type=='Inlet']  <- "light green"
     #head(trap.set.yr1)
...
```{r appendixH, echo=FALSE}
   ...
   trap.Usage    <-  read.csv(paste(path,'appendixH.csv',sep=''),header=T)
           ...
           trap.set.yr2         <-  srvy.set.details[ ,c(-1,-4:-18), ]
           addition         <-  c(8,"StRS")
           addition2        <-  c(11,"StRS")
...
```{r appendixJ, echo=FALSE}
 ...
 # add in sparkline graph count by trap
   ...
   colnames(surveyspec2) <- c("", "kg","Count","Count by Trap","Recover-Rerelease", "Sampled", "Released", 
                              "Count", "Mean", "Fork Length", "Sex","Maturity", "Otoliths",
                              "Weight","Count","Proportion Males", "Males", "Females" )
