...
(ref:figure1) Location of the boundaries of the offshore indexing localities (2010), mainland inlet localities (2010-2011),  exploratory sites (2011) and the five spatial areas (S~1~-S~5~) of the stratified random survey design (2010-2011). The three depths strata (RD~1~-RD~3~) are colour-coded and nested within each of the five spatial strata. 
...
(ref:figure2)  Start locations of the `r yr` StRS sets (red markers) conducted in spatial stratum areas S~1~ through S~5~ and standardized sets (yellow markers) conducted at offshore indexing localities.

```{r figure2, fig.cap='(ref:figure2)', out.width = "450px", out.height = "580px", fig.align="center", warning=FALSE, echo=FALSE, message=FALSE, error=FALSE}
      img <- paste(figpath,'Figure2.png'  ,sep="")
      knitr::include_graphics(img)
```
\clearpage

(ref:figure3)  Start locations of the `r yr+1` StRS sets (red markers) and exploratory sets (purple markers) conducted in spatial stratum areas S~1~ through S~5~.

```{r figure3, fig.cap='(ref:figure3)', out.width = "450px", out.height = "580px", fig.align="center", warning=FALSE, echo=FALSE, message=FALSE, error=FALSE}
      img <- paste(figpath,'Figure3.png'  ,sep="")
      knitr::include_graphics(img)
```
\clearpage

(ref:figure4) Location of the `r yr` and `r yr+1` standardized sets within the mainland inlet localities.  
```{r figure4, fig.cap='(ref:figure4)', out.width = "440px", out.height = "570px", echo=FALSE, fig.align="center", warning=FALSE}
...
(ref:figure7defunct) Sablefish catch per unit effort (CPUE) by depth and year for StRS sets. Dashed lines delineate depth strata (shallow(RD~1~) = 100-450 m, mid(RD~2~) = 450-850 m, deep(RD~3~) = 850-1400 m).

```{r figure7defunct,echo=FALSE, warning=FALSE}
    par(mfcol=c(3,3))
     #  commented out the figure for this paper, left the code
     #   results='asis',echo=FALSE,include=FALSE,warning=FALSE,message = FALSE,error= FALSE
     #   png(paste(figpath,'figure11.png', sep=""), width = 448, height = 628)  # write png to folder
...
```{r figure7, fig.cap='(ref:figure7)', out.width = "450px", out.height = "576px",echo=FALSE, fig.align="center", warning=FALSE}
     ...
     png(paste(figpath,'figure7.png',sep=''), width=500, height=641) # write png to file
     par(mfcol=c(1,1)) 
     tgc.02 <- tgc.02[tgc.02$SET_YEAR<2012,]
     tgc.02$col <- ifelse(tgc.02$SET_YEAR %in% c(2010,2011), "red", "steelblue")
...
```{r figure8, fig.cap='(ref:figure8)', out.width = "450px", out.height = "576px",echo=FALSE, fig.align="center", warning=FALSE}
    ...
    png(paste(figpath,'figure8.png', sep=''), width=500, height=641) # write png to file
    par(mfcol=c(1,1))
    
    raw_survey_data        <- raw_survey_data[raw_survey_data$SET_YEAR<2012,]
    ...
    tgcnt$col <- ifelse(tgcnt$SET_YEAR %in% c(2011,2010), "red", "steelblue")
...
(ref:figure9) Average weight of sablefish (mean +/- 95% CIs) by StRS survey strata over time. Panels run deep to shallow (left to right) and north to south (top to bottom).

```{r figure9, fig.cap='(ref:figure9)', out.width = "450px", out.height = "576px",echo=FALSE, fig.align="center", warning=FALSE}

    png(paste(figpath,'figure9.png',sep=''), width=500, height=641) # write png to file
    par(mfcol=c(1,1))

    tgcwg <- tgcwg[tgcwg$SET_YEAR<2013,]
    tgcwg$col <- ifelse(tgcwg$SET_YEAR %in% c(2011,2010), "red", "steelblue")
...
(ref:figure11) Offshore indexing site annual mean weight of sablefish per trap (kg/trap) (A);  annual mean number of sablefish per trap (#fish/trap) (B); annual mean weight of sablefish (kg) (C) over time. Horizontal line is median and blue dots are arithmetic mean.
...
#------S Q L ---C P U E ----B O X P L O T ---O F F S H O R E---S T A N D A R D I Z E D---------
    ...
    offstnd.dat       <- offstnd.dat[offstnd.dat$year >=1990 & offstnd.dat$year <=2010,]  
    ...
    offstnd.dat$sable_cpuewg <-  offstnd.dat$CPUE.SABLE.WEIGHT/offstnd.dat$CPUE.SABLE.COUNT
    ...
    offstand.2wg             <-  arrange(transform(offstnd.dat[offstnd.dat$year>1989,],
     ...
     g <- ggplot(offstand.2w, aes(x=year, y=CPUE.SABLE.WEIGHT/CPUE.TRAPS),
                 ...
                 scale_x_discrete(breaks = seq(1990, 2010, by = 2)) +
...
```{r figure12, fig.cap='(ref:figure12)', results='asis', out.width = "450px",out.height = "560px", fig.align="center"}
    ...
    cpuedt    <-  "SELECT  * from dbo.GENERIC_GFBIO_TRAPS where year < 2012 and [Spatial.Stratum] not like '%Quatsino Sound%' "
    #cpuedat   <-  GetSQLData(cpuedt,"Sablefish") 
    #write.table(cpuedat, file = paste(path,"figure12.csv",sep=''),row.names=FALSE, na="",col.names=TRUE, sep=",")
    cpuedat   <-  read.csv(paste(path,'figure12.csv',sep=''),header=T)


    #pick out the stand sets
    dat   <- cpuedat[ which(cpuedat$SABLE.SET.TYPE=="INLET STANDARDIZED" ), ]
    dat   <- dat[ which(is.na(dat$SABLE.SET.TYPE)==FALSE), ]
    inlet <- dat[dat$year >1993 & dat$year <=2011,]  
    ...
    inlet.2wg         <-  arrange(transform(inlet[inlet$year>1993,],year=factor(year,levels=c(1994,1995,1996,1997,
     ...
     p <- ggplot(inlet.2w, aes(x=year, y=CPUE.SABLE.WEIGHT/CPUE.TRAPS)) +
          ...
          ylab("CPUE (kg/trap)") +
          #scale_y_continuous(limits=c(0,100))+
          scale_x_discrete(breaks = seq(1994, 2011, by = 5)) +
...
```{r figure13, fig.cap='(ref:figure13)',results='asis', out.width = "450px",out.height = "550px", fig.align="center"}
    ...
    # unique(dat$SABLE_SET_TYPE)   # should be only StRS
    ...
    plot(fdat$YEAR, fdat$AvL,pch=2, xlim=c(2003,yr+1), ylim=c(50, 78),cex=1.0, 
          ...
          col="gray30",xaxt="n")
    axis(1, seq(2003,2011,1), cex.axis=1.0)
...
(ref:figure14) Length frequencies for female (grey) and male sablefish (blue) up to `r yr` for all offshore indexing sets.  The number of specimens is denoted by the letter n, the mean indicated by xbar $\overline{x}$ and the standard deviation is represented by sigma $\sigma$ (A). Average length and ratios of male and female sablefish by year. Numbers of fish are shown across the top of the lines (B).
...
```{r figure14, fig.cap='(ref:figure14)',results='asis', out.width = "450px",out.height = "550px", fig.align="center"}
    ...
    knitr::opts_chunk$set(out.height = "\\textheight",  out.width = "\\textwidth")
      ...
      panLab(0.83,0.4,paste("Offshore Indexing 1990-", yr, sep=""), cex=1.0)  # -- Labels unique to 2010
    	panLab(0.19,0.98,"Sablefish males", col="steel blue")
    	panLab(0.79,0.98,"Sablefish females", col= "gray50")
     ...
     dat.lenstats.Off  <- dat.lenstats.Off[dat.lenstats.Off$YEAR <= yr, ]  # unique to 2010
     ...
     plot(fdat.lenstats.Off$YEAR, fdat.lenstats.Off$AvL,pch=2, xlim=c(1990,yr), ylim=c(50, 78),cex=1.0, 
          ylab="Average length (cm)", xlab="Year", cex.lab=1.0, cex.axis = 1.0,
          col="gray30",xaxt="n")
     axis(1, seq(1990,2010,1), cex.axis=1.0)
     ...
     legend(0.035,1, c("  Sablefish females","  Sablefish males"), 
            lty=c(2,1), lwd=2, bty="n", col=c("gray50","steelblue"), cex=1.0)
     legend(0.025,1, c("",""), pch=c(2,16), bty="n", col=c("gray50","steelblue"))
     #panLab(0.76,0.94,paste("Offshore Indexing 1990-", yr, sep=""), cex=1.0)
     ...
     while (!is.null(dev.list()))  dev.off()  
     img13 <- paste(paste(figpath,'figure14.png',sep='')) 
     knitr::include_graphics(img13)
``` 


            
(ref:figure15) Sablefish fork length (cm) vs weight (kg) for females and males for the `r yr` (A,C) and `r yr+1` (B,D) surveys.
...
```{r figure15, fig.cap='(ref:figure15)',results='asis', out.width = "450px",out.height = "550px", fig.align="center"}
    ...
    #write.table(lenWTdat1, file = paste(path,"figure15.csv",sep=''), row.names=FALSE, na="",col.names=TRUE,  sep=",") # read from SQL Server
...
```{r figure17, fig.cap='(ref:figure17)',results='asis', out.width = "450px",out.height = "570px", fig.align="center"}
   ...
   # -- plot bubbles for females
   plot(dat.age.off$Year, dat.age.off$Age, type="n",xlab=" Year", 
        ylab="", main="Females", ylim=c(0,42), xlim=c(minyr,maxyr),
        xaxt="n")
        axis(1, seq(1990,2010,1))            
   symbols(dat.age.off$Year, dat.age.off$Age, circles=pi*(dat.age.off$Female_Proportion),
...
```{r figure18, fig.cap='(ref:figure18)',results='asis', out.width = "500px",out.height = "600px", fig.align="center"}
   ...
   png(paste(figpath,'figure18a.png', sep=''), width = 920, height = 424)  # -- starts writing a png to figure folder
     ...
     image_write(side_by_side, path = "C:/github/surveyreport_201011/figures/Figure18.png", format = "png")  # save png
...
```{r figure19, fig.cap='(ref:figure19)', results='asis', out.width = "430px",out.height = "466px", fig.align = "center"}
      ...
      sea.bird           <-  "select * from SEABIRD_AverageTempPerSet where depth_stratum in('RD1','RD2','RD3') and Year < 2012"
